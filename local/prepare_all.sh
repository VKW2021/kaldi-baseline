...
if [ $# -ne 3 ]; then
  ...
  echo " e.g prepare_all.sh /data/VKW/train /data/VKW/dev /data/VKW/test"
  #exit 1;
fi
...
#put lexicon.txt to "data/local/dict/" first, then run the scripts
if [ $stage -le 1 ]; then
...
if [ $stage -le 3 ]; then
  utils/prepare_lang.sh --position-dependent-phones false \
    data/local/dict "<UNK>" data/local/lang data/lang || exit 1; # <UNK>, <SPOKEN_NOISE>
fi
