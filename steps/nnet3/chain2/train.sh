...
kaldi=/apdcephfs/share_1157259/users/janinezhao/kaldi/egs/vkw_ddt/s5/
devcloud=/apdcephfs/share_1157259/users/janinezhao/devcloud_kaldi

stage=-2
cmd=$kaldi/utils/run.pl
gpu_cmd_opt=
...
if [ -f $devcloud/path.sh ]; then . $devcloud/path.sh; fi
. $kaldi/utils/parse_options.sh || exit 1;
...
$kaldi/steps/chain2/validate_randomized_egs.sh $egs_dir
...
if [ $stage -le -2 ]; then
    ...
    echo "PATH:" $PATH
    $kaldi/steps/chain2/internal/get_train_schedule.py \
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 1 ]; then
      ...
      model_index=`/apdcephfs/share_1157259/users/janinezhao/kaldi/egs/librispeech/s5/steps/nnet3/chain2/internal/get_best_model.sh --output output-${lang} $dir/log/train.$x.*.log`
