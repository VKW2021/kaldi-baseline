...
use_icu=false #true zhaojing changed
...
help_message="
   Usage: local_kws/kws_data_prep.sh <lang-dir> <data-dir> <kws-data-dir>
    e.g.: local_kws/kws_data_prep.sh data/lang/ data/eval/ data/kws/
...
if  $case_insensitive && ! $use_icu  ; then
  ...
  paste <(cut -f 1  $kwsdatadir/keywords.txt  ) \
    ...
    local_kws/kwords2indices.pl --map-oov 0  $kwsdatadir/words.txt > $kwsdatadir/keywords_all.int
else
...
(cut -f 1 -d ' ' $kwsdatadir/keywords.int | \
  local_kws/subset_kwslist.pl $keywords > $kwsdatadir/kwlist_invocab.xml) || true

(cat $kwsdatadir/keywords_all.int | \
  egrep " 0 | 0$" | cut -f 1 -d ' ' | \
  local_kws/subset_kwslist.pl $keywords > $kwsdatadir/kwlist_outvocab.xml) || true
